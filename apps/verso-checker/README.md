...
- **`SPECS_AI_BASE_URL`** must point at a host:port the Verso **Node process** can reach (same machine, Docker gateway, or routed LAN).
- If Verso runs in **Docker** and the LLM is on another host, the container must be able to route to that address (often **`extra_hosts`**, **host** network for `web`, or run the stack on the LLM host). A bridge-only container may **not** reach arbitrary LAN IPs.
...
3. Prefer a **`VITE_VERSO_API_BASE`** (e.g. `http://localhost:1337`) in `apps/verso-checker/.env` for dev; never bake secrets into the client — the public **`POST /api/public/laptop-specs`** is already rate-limited but not a secret channel.
