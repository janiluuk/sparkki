...
Ongoing priorities from **`ROADMAP.md`** (Review backlog): CSP **baseline** is in repo (report-only and/or enforcing via env; optional **`report-uri`** → **`/api/csp-report`** — see **`docs/operations.md`**). **Optional next:** stricter **`script-src`** (nonces/hashes). For Docker **`web`** builds, Compose passes **`DATABASE_URL`** via **`host.docker.internal`**; use **`npm run docker:build:web`** or see **`docs/repository-layout.md`** § Known sharp edges. Then pick from **`FEATURES.md`** by priority table.
