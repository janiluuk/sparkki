...
6. **Production image build + Prisma** — **Mitigation shipped:** **`docker-compose.yml`** passes a build-time **`DATABASE_URL`** (default via **`host.docker.internal`** + published **`POSTGRES_PORT`**) and **`scripts/docker-build-web.sh`** starts **`db`** then builds **`web`**. See **`docs/repository-layout.md`** § Known sharp edges. Optional: **`DATABASE_URL_BUILD`** in **`.env`**; or move DB-bound SSG to **`dynamic = 'force-dynamic'`** / client fetch if you cannot expose Postgres during build.
